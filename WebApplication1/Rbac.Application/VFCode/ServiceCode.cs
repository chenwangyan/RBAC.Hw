...
namespace Rbac.Application.VFCode
{
    public class ServiceCode : IServiceCode
    {
        ...
        public ServiceCode(IHttpContextAccessor accessor)
        {
            this.accessor = accessor;
        }

        public Task<CodeResult> GenerateCaptchaImageAsync(string captchaCode, int width = 0, int height = 30)
        {
            //验证码颜色集合
        ...
        public Task<string> GenerateRandomCaptchaAsync(int codeLength = 4)
        {
            ...
            //session.SetSession("CodeKey",t.Result);
